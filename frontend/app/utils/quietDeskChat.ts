...
export function quietDeskReply(question: string): QuietDeskReply {
  ...
  if (/background|about|experience|who.*lucas|backend|architecture|frontend|\bai\b/.test(text)) {
    return { text: 'Lucas is a software engineer focused mainly on backend work and architecture, with some frontend experience and a growing interest in AI.' }
  }
  if (/approach|work|process|design/.test(text)) {
    return { text: 'His approach is to understand the context, shape a clear system around real constraints, and iterate carefully.' }
  }
  ...
  return { text: 'I only have a small set of verified information about Lucas, so I can help with his background, approach, skills, or contact links.' }
}
